...
from utils.TCPutils import send_json, recv_json, ConnectionClosedByPeer ,create_tcp_socket
...
class ClientFSM:
    ...
    def start(self, host="127.0.0.1", port=9000):
        ...
        # push event for fsm
        self.event_queue.put(("connect", None))

        self.u_listener = threading.Thread(target=self.server_listener, daemon=True)
        self.u_listener.start()
        self.s_listener = threading.Thread(target=self.user_listener, daemon=True)
        self.s_listener.start()

        self.fsm_loop()

        self.u_listener.join()
        self.s_listener.join()
    ...
    def register(self, data):
        send_json(self.sock, {"op": "register", "name": data['name'], "passwordHash": data["passwordHash"]})
    ...
    def create_room(self, data):
        send_json(self.sock, {"op": "create_room", "id": self.user_id, "name": data["room_name"] , "visibility": data["visibility"]})
